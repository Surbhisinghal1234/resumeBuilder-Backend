...
// app.use(cors({ origin: "https://resumebuilder-o05w.onrender.com", credentials:true }));
...
mongoose.connect(
    `mongodb+srv://${username}:${password}@cluster0.3j0ywmp.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0/resumeBuilder`
)
    .then(() => {
        console.log("MongoDB connected");
        app.listen(port, () => {
...
apiRouter.post("/send", upload.single("image"), async (req, res) => {
    try {
        console.log("Body", req.body);
        console.log("File", req.file);
        const { dataSave } = req.body;
...
apiRouter.post("/register", async (req, res) => {
    ...
    try {
        ...
        // jwt 
        const token = jwt.sign({ id: newUser._id, email: email }, "surbhi1234", { expiresIn: '2h' });
...
apiRouter.post("/login", async (req, res) => {
    ...
    try {
        ...
        // jwt
        const token = jwt.sign({ id: user._id, email: email }, "surbhi1234", { expiresIn: '2h' });
        console.log(token, "token");
        res.cookie('token', token, { httpOnly: true, secure: process.env.NODE_ENV === 'production' });
...
// delete many
apiRouter.delete("/deleteMany", async (req, res) => {
    try {
        const { ids } = req.body;
        const result = await dbModel.deleteMany({ _id: { $in: ids } });
        res.status(200).json({ message: "Successfully deleted", result });
...
apiRouter.get("/getById/:id", async (req, res) => {
    const idToGet = req.params.id;
    try {
        const resumeData = await dbModel.findOne({ _id: idToGet });
        const userData = await RegisterModel.findOne({ _id: idToGet });
...
apiRouter.put("/update/:id", upload.single('image'), async (req, res) => {
    const idToUpdate = req.params.id;
...
// Use the /api prefix for the apiRouter
